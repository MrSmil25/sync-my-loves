...
/** Program dibatalkan → tagihannya disembunyikan dari anggota & antrian verifikasi. */
function isCancelled(p: CollectionPayment) {
  return p.collections?.status === "Dibatalkan";
}
...
export async function fetchMyBills(): Promise<CollectionPayment[]> {
  ...
  if (error) throw error;
  return ((data ?? []) as CollectionPayment[]).filter((p) => !isCancelled(p));
}
